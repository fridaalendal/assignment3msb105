...
plot_pendlere <- function(data, bosted_kode, grense = 1) {
  ...
  # Finn navn på bostedet (f.eks. "Stavanger")
  bosted_navn <- data |>
  ...
  data |>
    # 1) Bare de som bor i valgt kommune
    dplyr::filter(bkom == bosted_kode) |>
    
    # 2) Vi vil se hvor de pendler UT, så fjern de som jobber i samme kommune
    dplyr::filter(akom != bkom) |>
    
    # 3) Ta bare med arbeidssteder som har minst grense % i minst ett år
...
stav_sand <- apd_0224_l |>
  dplyr::filter(
    akom == bkom,                    # bor og jobber i samme kommune
    bkom %in% c("k1103", "k1108")    # Stavanger og Sandnes
  ) |>
  dplyr::mutate(Bosted = bkom_navn)  # pen etikett i legenda
...
plot_pendlere(apd_0224_l, "k1124", grense = 1.7) +
  ylim(5, 40)   # valgfritt, men da ligner det veldig på Arnstein sitt
