...
```{r}
# Kommuner i BA-region 49 (Stavanger/Sandnes)
...
```{r}
pend_49_2020 <- pendle_data_02_24 |>
  filter(
    aar == "2020",
    bkom %in% c(reg49_kom, "k9999"),
    akom %in% c(reg49_kom, "k9999")
  )
...
mat_49_2020_tab <- mat_49_2020 |>
  ...
  arrange(bkom) |>
  flextable() |>
  colformat_int(big.mark = " ") |>   # mellomrom som tusenskiller
...
# kommunene i region 49 (alle unntatt 9999)
komm <- mat_49_2020$bkom[mat_49_2020$bkom != "9999"]

# ut av regionen (bor i kommunen, jobber i 9999)
ut <- mat_49_2020 |>
  filter(bkom %in% komm) |>
  select(bkom, `9999`) |>
  rename(ut = `9999`)

# inn til kommunen (bor i 9999, jobber i kommunen)
inn <- mat_49_2020 |>
  filter(bkom == "9999") |>
  select(-bkom) |>
