...
```{r}
#| label: tbl-pmat-2010
#| tbl-cap: Pendlematrise for Bokna-regionen 2010.
#| echo: false
pendlematrise_2010 %>% 
  as_flextable(max_row = 30, show_coltype = FALSE) |> 
  fontsize(size = 9) |> 
...
```{r}
#| label: tbl-pmat-2017
#| tbl-cap: Pendlematrise for 2017 for Bokna-regionen. Data fra [SSB](https://www.ssb.no/) statistikktabellene [03321](https://www.ssb.no/statbank/table/03321) og [11616](https://www.ssb.no/statbank/table/11616).
#| echo: false
tab |> 
  as_flextable(
    show_coltype = FALSE,
...
```{r}
#| label: tbl-pmat-49-2020
#| tbl-cap: Pendlematrise for region 49, 2020.
mat_49_2020_tab
```
...
kommuner_flere_ut |> 
  (\(x) {
  paste(
    "Kommuner hvor flere pendler ut enn inn: ", 
    paste(x, collapse = ", "
          )
  )
  })()
...
```{r}
tab_pendlere <- function(data, knr, y = 2023, n = 7) {
...
andel_samme_kommune_2023 <- apd_0224_l |>
  dplyr::filter(
    aar == as.Date("2023-01-01"),   
...
```{r}
andel_samme_kommune_2023_tab <- andel_samme_kommune_2023 |>
  knitr::kable(
    digits  = 1,
    caption = "Tabell som viser andelen av arbeidstakere som jobber i samme kommune som hvor de bor i 2023.",
    booktabs = TRUE
  )
```

```{r}
andel_samme_kommune_2023_tab
```
```{r}
#| label: tbl-andel-internpend-2023
#| tbl-cap: Andelene av arbeidstakere som bor og jobber i samme kommune i 2023.
# ag: Hvordan jeg ville laget tabellen.
 andel_samme_kommune_2023 |> 
  filter(!akom %in% c("k0000", "k9999")) |> 
  arrange(akom) |> 
  select(
    `Kommune-\nnummer` = akom, 
    `Kommune-\nnavn` = akom_navn,
  ...
  mutate(
    Andel = paste0(as.character(round(Andel, 1)),"%")
  ) |> 
  as_flextable(
    show_coltype = FALSE,
    max_row = 30
...
I @tbl-andel-internpend-2023 er andelen internpendlere rapportert for kommunene i Bokna-regionen.

```{r}
intern_utvalg <- apd_0224_l |>
  dplyr::filter(
    akom == bkom,
    !(akom == "k9999" & bkom == "k9999"),
    !(akom == "k0000" & bkom == "k0000"),
    bkom_navn %in% c("Hjelmeland", "Hå", "Kvitsøy", "Stavanger", "Strand")
  )


legend_labels <- intern_utvalg |>
  dplyr::distinct(bkom, bkom_navn) |>
  dplyr::arrange(bkom)

ggplot(
  intern_utvalg,
  aes(x = aar, y = andel, group = bkom, colour = bkom)
) +
  geom_line(linewidth = 0.75) +
  scale_colour_discrete(
    name   = "Bosted",
    breaks = legend_labels$bkom,
    labels = legend_labels$bkom_navn
  ) +
  labs(
    title = "Internpendling et utvalg kommuner",
    x     = "aar",
    y     = "andel"
  ) +
  theme(
...
```{r}
plot_pendlere <- function(data, bosted_kode, grense = 1) {
  
  
  bosted_navn <- data |>
    dplyr::filter(bkom == bosted_kode) |>
    dplyr::distinct(bkom_navn) |>
    dplyr::pull()
  
  data |>
    dplyr::filter(bkom == bosted_kode) |>
    dplyr::filter(akom != bkom) |>
    
    dplyr::group_by(akom, akom_navn) |>
    dplyr::filter(max(andel, na.rm = TRUE) >= grense) |>
    dplyr::ungroup() |>
  
    ggplot(aes(x = aar, y = andel, colour = akom_navn, group = akom_navn)) +
    geom_line(linewidth = 0.75) +
    labs(
      title  = paste0("Hvor jobber folk bosatt i ", bosted_navn, " kommune?"),
      x      = "aar",
...
```{r}
tab_pendlere(apd_0224_l, "k1103") |>
  colformat_double(digits = 1)
```

```{r}
#| label: fig-intern-1103-1008
#| fig-cap: "Utvikling: Andel som både bor og jobber i Stavanger og Sandnes."
stav_sand <- apd_0224_l |>
  dplyr::filter(
    akom == bkom,                    
    bkom %in% c("k1103", "k1108")    
  ) |>
  dplyr::mutate(Bosted = bkom_navn)  

ggplot(stav_sand, aes(x = aar, y = andel, colour = Bosted)) +
  geom_line(linewidth = 0.75) +
  labs(
    #title = "Utvikling: Andel som både bor og jobber i Stavanger og Sandnes",
    x     = "År",
    y     = "Andel av arbeidstakere",
    colour = "Arbeidssted"
  ) +
...
plot_pendlere <- function(data, bosted_kode, grense = 1) {
  ...
  data |>
    ...
    ggplot(aes(x = aar, y = andel,
               colour = akom_navn, group = akom_navn)) +
    geom_line(linewidth = 0.75) +
    labs(
      title  = paste0("Hvor pendler folk bosatt i ", bosted_navn, " kommune til?"),
      x      = "aar",
      y      = "Andel av arbeidstakere",
      colour = "Arbeidssted"
    ) +
...
```{r}
#| warning: false
plot_pendlere(apd_0224_l, "k1124", grense = 1.7) +
  ylim(5, 40)   
...
plot_pendlere(apd_0224_l, "k1120", grense = 3) +
  ylim(0, 18) +    
  theme(
    legend.position = "bottom",
    plot.title = element_text(face = "bold", size = 18)
...
tab_pendlere(apd_0224_l, "k1120", n = 7) |>
  colformat_double(digits = 1)
```

```{r}

plot_pendlere(apd_0224_l, "k1106", grense = 2) +
  ylim(0, 15) +   
  theme(
    legend.position = "bottom",
    plot.title = element_text(face = "bold", size = 18)
...
tab_pendlere(apd_0224_l, "k1106", n = 7) |>
  colformat_double(digits = 1)
```
...
```{r}
pend_mat_tab <- function(data, y) {
  mat = pend_mat(data, y)
  mat = flextable::as_flextable(mat, show_coltype = FALSE, max_row = 30)
  mat = flextable::colformat_num(mat, big.mark = " ")
  mat = flextable::fontsize(mat, size = 10, part = "all")
  mat = flextable::line_spacing(mat, space = 0.5)
  mat = flextable::align(mat, align = "center", part = "all")
  mat = flextable::autofit(mat)
  mat = flextable::delete_part(mat, "footer")
  # mat = flextable::set_caption(mat,
  #     if (is.null(caption)) paste0(caption, y, ".") else caption
  #   )
  mat
}
```
...
I @tbl-pmat-2002, @tbl-pmat-2005, @tbl-pmat-2010, @tbl-pmat-2015, @tbl-pmat-2023 og @tbl-pmat-2024 er gjengitt pendlematriser for Bokna-regionen for årene 2002, 2005, 2010B, 2015, 2023 og 2024.

:::{.landscape}

```{r}
#| label: tbl-pmat-2002
#| tbl-cap: Pendlematrise år 2002.
#| echo: false
pend_mat_tab(pendle_data_02_24, y = 2002) 
```


```{r}
#| label: tbl-pmat-2005
#| tbl-cap: Pendlematrise Bokna-regionen 2005.
#| echo: false
pend_mat_tab(pendle_data_02_24, y = 2005)
```
...
```{r}
#| label: tbl-pmat-2010B
#| tbl-cap: Pendlematrise Bokna-regionen 2010.
#| echo: false
pend_mat_tab(pendle_data_02_24, 2010)
```


```{r}
#| label: tbl-pmat-2015
#| tbl-cap: Pendlematrise Bokna-regionen 2015.
#| echo: false
pend_mat_tab(pendle_data_02_24, 2015)
```
:::
...
```{r}
#| label: tbl-pmat-2023
#| tbl-cap: Pendlematrise Bokna-regionen 2023.
#| echo: false
pend_mat_tab(pendle_data_02_24, 2023)
```


```{r}
#| label: tbl-pmat-2024
#| tbl-cap: Pendlematrise Bokna-regionen 2024.
#| echo: false
pend_mat_tab(pendle_data_02_24, 2024)
```
